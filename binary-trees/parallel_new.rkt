...
(module+ main
  (define (main n)
    ...
    (define channels (for/list ([c (in-list (cdr intervals))])
                       (define ch (make-checking-place))
                       (for/list ([d (in-list c)])
                         (define iterations (arithmetic-shift 1 (+ (- max-depth d) min-depth))) ;arithmetic-shift candidate for unsafe op?
                         (place-channel-put ch iterations)
                         (place-channel-put ch d)
                         (list iterations d ch))))
    (define channelsx (cons (let ([c (car intervals)])
                              (for/list ([d (in-list c)])
  ...
  (optimization-coach-profile (main 21))
  ;(main 21)
)